...
class NewsSummaryEvaluator:
    def __init__(self):
        ...
        print("✅ DeepSeek Flash+Pro 双评分矩阵就绪！")
    ...
    def evaluate_factuality(self, source: str, generated: str, sample_id: int) -> float:
        ...
        请先简明扼要地列出你的核查分析过程。最后一行独立使用 <score>最终分数</score> 给出打分（0-100整数）。
        ...
        if len(valid_scores) == 2:
            ...
            with print_lock:
                print(f"🚨 [样本 {sample_id}] Flash+Pro 全部失败，兜底 50 分。")
            return 50.0
...
if __name__ == "__main__":
    PREDICT_FILE = "/root/autodl-tmp/LLaMA-Factory/outputs/base2/generated_predictions.jsonl"
    evaluator = NewsSummaryEvaluator()
    news_sources, references, generateds = [], [], []
    with open(PREDICT_FILE, 'r', encoding='utf-8') as f:
        for line in f:
            data = json.loads(line)
            news_sources.append(data.get("prompt", ""))
            references.append(data.get("label", ""))
            generateds.append(data.get("predict", ""))
    total_samples = len(generateds)
    print("🧠 批量计算 BERTScore ...")
    ...
    def process_single_sample(i):
        ...
        with print_lock:
            ...
            if global_counter % 100 == 0:
                ...
                print(f"\n{'-'*15} 🕒 已完成 {global_counter}/{total_samples} {'-'*15}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        list(tqdm(executor.map(process_single_sample, range(total_samples)), total=total_samples, desc="Base 评测进度"))

    print("\n" + "="*50)
    print(f"🏆 Base 模型评测报告 (样本数: {total_samples})")
    ...
    print(f"🧠 平均忠实度: {np.mean(valid_fact) if valid_fact else 0:.2f}")
